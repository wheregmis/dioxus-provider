...
pub struct MutationData<O: Clone + PartialEq + 'static, E: Clone + PartialEq + 'static> {
    ...
    pub error: Option<E>,
    /// Whether this is an optimistic update (will be replaced by actual result)
...
pub struct Mutation<I: 'static, O: Clone + PartialEq + 'static, E: Clone + PartialEq + 'static> {
    ...
    /// Optimistic update function (applies before mutation completes)
    #[allow(clippy::type_complexity)]
    pub(crate) optimistic_fn: Signal<Option<Box<dyn Fn(&I, &ProviderCache) -> Option<O> + 'static>>>,
    /// Cache reference
    #[allow(dead_code)]
...
impl<I: 'static, O: Clone + PartialEq + 'static, E: Clone + PartialEq + 'static> Mutation<I, O, E> {
    ...
    /// Invalidate a specific typed key.
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let key = fetch_user.ty_key(&(123,));
    /// use_mutation(update_user).invalidate_key(key);
    /// ```
    pub fn invalidate_key<T>(mut self, key: crate::cache::TypedKey<T>) -> Self {
        self.invalidation_keys.write().push(key.key);
        self
    }

    /// Invalidate all cache entries for a specific provider function.
    ///
    /// This uses the provider's root key prefix to find and remove all matching entries.
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// use_mutation(clear_users).invalidate_all(fetch_user);
    /// ```
    pub fn invalidate_all<F, M, E2>(mut self, provider_fn: F) -> Self
    where
        F: ProviderCallback<M, E2> + 'static,
        M: 'static,
        E2: 'static,
    {
        let root = provider_fn.root_key();
        // We use a special prefix marker to tell the runtime to do a prefix match
        self.invalidation_keys.write().push(format!("prefix:{}", root));
        self
    }

    /// Set an optimistic update function.
    ///
    /// The function receives the input and should return an optimistic result
    /// that will be shown immediately while the mutation executes in the background.
    ...
    /// let mut update = use_mutation(update_user)
    ///     .optimistic(|input| {
    ///         // Return optimistic result based on input
    ///         User { id: input.0, name: input.1.clone(), ..Default::default() }
    ///     });
    /// ```
    pub fn optimistic<Func>(mut self, func: Func) -> Self
    where
        Func: Fn(&I) -> O + 'static,
    {
        self.optimistic_fn.write().replace(Box::new(move |input, _| {
            Some(func(input))
        }));
        self
    }

    /// Set an optimistic update function that can also modify the global cache.
    ///
    /// The function receives the input and the global cache, allowing you to
    /// update other providers' data immediately. It can optionally return an
    /// optimistic result for the mutation itself.
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// let mut update = use_mutation(delete_item)
    ///     .on_optimistic_update(|input, cache| {
    ///         // Remove item from the list immediately
    ///         let id = input.0;
    ///         if let Some(mut items) = cache.get::<Vec<Item>>("load_items") {
    ///             items.retain(|i| i.id != id);
    ///             cache.set("load_items", items);
    ///         }
    ///         // Return optimistic result for the mutation (optional)
    ///         Some(id)
    ///     });
    /// ```
    pub fn on_optimistic_update<Func>(mut self, func: Func) -> Self
    where
        Func: Fn(&I, &ProviderCache) -> Option<O> + 'static,
    {
        self.optimistic_fn.write().replace(Box::new(func));
        self
    }
    ...
    O: Clone + Send + Sync + PartialEq + 'static,
{
    ...
    let callback: Signal<Box<dyn Fn(F::Input) + 'static>> =
        use_signal(move || -> Box<dyn Fn(F::Input) + 'static> {
            ...
            let optimistic_fn = optimistic_fn;
            ...
            Box::new(move |input: F::Input| {
                ...
                if let Some(opt_fn) = optimistic_fn.read().as_ref() {
                    if let Some(optimistic_value) = opt_fn(&input, &cache) {
                        store.value().set(Some(optimistic_value));
                ...
                spawn(async move {
                    ...
                    match result {
                        ...
                        Err(e) => {
                            ...
                            crate::debug_log!("❌ [MUTATION] Failed, rolled back optimistic update");
                        }
                    }
                    ...
                    for key in invalidation_keys.iter() {
                        if let Some(prefix) = key.strip_prefix("prefix:") {
                            // Prefix invalidation
                            crate::debug_log!("🔄 [MUTATION] Invalidating all keys with prefix: {}", prefix);
                            
                            // We need to find all keys starting with this prefix
                            // This requires a scan of the cache keys
